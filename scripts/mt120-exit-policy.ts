/**
 * MT120 — an exit POLICY instead of an exit CLOCK.
 *
 * Frozen in `docs/MULTIPLE_TESTING_LEDGER.csv` row MT120 before this file existed, grid and all.
 *
 * Every measurement this programme has made exits on a fixed clock. MT110's shape says why that
 * matters: median -0.03% sitting on a mean of -2.39%, and MT117 put p05 near -35%. A distribution
 * whose median is near zero and whose mean is deeply negative is one where a few catastrophic
 * holds do all the damage — the shape a stop removes. MT118 sharpened it: the EXIT dominates.
 *
 * THE GRID IS FROZEN AND SMALL, because multiplicity is what killed MT119: TP in {50,100,200} bps,
 * SL in {200,400} bps, timeout 60s. Six cells plus the fixed-15s baseline. Nothing may be re-tuned.
 *
 * THE MEAN IS REPORTED BESIDE THE MEDIAN throughout, because the whole hypothesis is about a tail
 * and a median cannot see a tail. A rule that lifts the median while worsening the mean has moved
 * the loss, not removed it, and is recorded as a failure.
 *
...
console.log('MT120 — exit policy vs exit clock\n');
...
const table = (label: string, m: Map<string, { rets: number[]; days: Map<string, number[]> }>): void => {
  console.log('');
  console.log(label);
  console.log('  policy          n       median       MEAN     %positive   days   daysPos');
  for (const k of KEYS) {
    const e = m.get(k);
    ...
    console.log(`  ${k.padEnd(14)} ${String(e.rets.length).padStart(6)} ${F(med(e.rets))}% ${F(mean(e.rets))}%   ${(100 * pos).toFixed(1).padStart(5)}%    ${String(e.days.size).padStart(2)}     ${dp}`);
  }
};
