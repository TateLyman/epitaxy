...
console.log('');
console.log(`MT149 — prior launch count as a signal, ${pools.length.toLocaleString()} pools with a real creator`);
...
for (const [label, f] of buckets) {
  const g = pools.filter((p) => f(priorOf.get(p.pool) ?? 0));
  if (g.length === 0) continue;
  const survived = g.filter((p) => p.trades >= JUDGE_AT_TRADE).length;
  const scored = g.map((p) => p.ret).filter((r): r is number => r !== null);
  console.log(
    `  ${label.padEnd(18)} ${String(g.length).padStart(7)}  ${(100 * survived / g.length).toFixed(1).padStart(9)}%  ${String(med(g.map((p) => p.trades))).padStart(13)}  ${String(scored.length).padStart(9)}  ` +
    `${(scored.length >= 30 ? mean(scored).toFixed(0) : 'n/a').padStart(10)}  ${(scored.length >= 30 ? med(scored).toFixed(0) : 'n/a').padStart(11)}`,
  );
}
console.log('');
