-- FETCHED FROM DUNE, not generated here. Query 8381294, execution
-- 01M0D9JFPTQBPJ02VSYZ9G9JJA, result ops/dune/results/q2-fit-decile-median-cut.json.
--
-- This is query 2 with ONE line changed, the decile cut:
--     NTILE(10) OVER (ORDER BY fw.mean_ret_fit   DESC)   ->
--     NTILE(10) OVER (ORDER BY fw.median_ret_fit DESC)
--
-- It exists because query 1 found the mean of the reconstructed return
-- contaminated by residual marks on open pumpswap positions, so the decile
-- gradient had to be re-cut on a statistic a single broken mark cannot move. It is
-- committed as the EXACT TEXT DUNE EXECUTED, pulled back from the API rather than
-- regenerated, because the source file has changed since it ran and a regenerated
-- file would be a different artifact from the one the committed result came from.
--
-- It is deliberately ABSENT from ops/dune/query-ids.json: `pnpm dune:assemble
-- --push` would then overwrite it with the mean-cut text on the next push.

-- VARIANT of Q2: deciles cut on the fit MEDIAN, not the fit mean.
-- The mean of ret_carryfwd is contaminated by residual marks on open pumpswap
-- positions (query 1, FIT pumpswap: mean +2.53 against a median of -0.23), and
-- the fit ranking is what decides decile membership. If the holdout gradient
-- survives a median cut, it is not that contamination.
...
ranked AS (
  SELECT
    ...
    NTILE(10) OVER (ORDER BY fw.median_ret_fit DESC)           AS fit_decile,
